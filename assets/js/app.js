...
function generateGraph(cy, text) {
  ...
  data.forEach(function(word, index, arr) {
    if (word === '' || word == ' ') arr.splice(index, 1);
  });
  ...
  data.forEach(function(word, index, arr) {
    ...
    word.forEach(function(letter, l_index, letters) {
      ...
      if (filtered[0] != undefined && l_index != 0 && previousRepeat) {
        ...
        var inc_edge = cy.edges(
          '[id = "' +
            filtering_saves[0].data('tag') +
            '_' +
            letter +
            '_' +
            filtering_saves_new[0].data('index') +
            '_' +
            l_index +
            '"]',
        );
      ...
      if (
        (check =
          filtered.length != cy.collection().length && previousRepeat == true)
      ) {
        ...
        if (l_index > 0) {
          ...
          cy.add({
            group: 'edges',
            data: {
              id: previous.letter + '_' + letter + '_' + index + '_' + l_index,
              left: previous.letter,
              right: letter,
              tag: 1,
              source:
                previous.letter + '_' + previous.index + '_' + previous.l_index,
              target: letter + '_' + index + '_' + l_index,
              search: 0,
            },
          });
        } else if (l_index == 0) {
...
function searchSubGraphSylabs(cy, text) {
  if (text.length > 0) var serached = cy.nodes('[tag = "' + text[0] + '"]');
  //console.log(serached);
  serached.forEach(function(ele, i, eles) {
    second = ele.neighborhood(
      '[tag = "' +
        text[1] +
        '"][left = "' +
        text[0] +
